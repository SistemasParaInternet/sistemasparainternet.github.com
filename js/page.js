...
function get_numero_repositorios_publicos(){
    $.getJSON('https://api.github.com/users/SistemasParaInternet/repos', function(resp) {
        $('#repositorios_publicos').html(resp.length);
    });
}

function get_numero_seguidores(){
    $.getJSON('https://api.github.com/users/SistemasParaInternet/followers', function(resp) {
        $('#github_seguidores').html(resp.length);
    });
}

function get_repositorios_atualizados(numero_repositorios){
    $.getJSON('https://api.github.com/users/SistemasParaInternet/repos?&sort=updated&per_page='+ numero_repositorios +'&page=1', function(resp) {
        if (resp.length > 0) {
